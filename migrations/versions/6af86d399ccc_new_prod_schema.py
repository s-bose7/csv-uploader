...
def upgrade() -> None:
    ...
    op.execute("""
        CREATE OR REPLACE FUNCTION compute_agent()
        RETURNS TRIGGER AS $$
        DECLARE
            org_category VARCHAR;
        BEGIN
            -- Get the category of the organization
            SELECT category INTO org_category FROM organizations WHERE id = NEW.organization_id;

            -- Increment the rank of all existing agents for the organization
            UPDATE agents
            SET rank = rank + 1
            WHERE organization_id = NEW.organization_id;

            -- Update the segment_name for all agents
            UPDATE agents
            SET segment_name = org_category || ' Default Agent ' || rank
            WHERE organization_id = NEW.organization_id;

            -- Insert the new agent with rank 1
            INSERT INTO agents (contact_id, organization_id, rank, club_id, segment_name)
            VALUES (NEW.id, NEW.organization_id, 1, NEW.club_id, org_category || ' Default Agent 1');
    ...
    op.execute("""
        ...
        FOR EACH ROW
        EXECUTE FUNCTION compute_agent();
    """)

    op.execute("""
        CREATE OR REPLACE FUNCTION update_agent_club_id()
        RETURNS TRIGGER AS $$
        BEGIN
            -- Update the agents.club_id based on the updated contacts.club_id
            UPDATE agents
    ...
    op.execute("""
        ...
        AFTER UPDATE OF club_id ON contacts
        FOR EACH ROW
        EXECUTE FUNCTION update_agent_club_id();
    """)
